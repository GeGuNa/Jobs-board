...
<?php 
	...
	if (isset($_GET['r_req'])) {
		...
		$pdo->query("delete from cv_requests where company_id = ? and id = ? and ",[$user['uid'], $us_id]);
...
$d_count = $pdo->query("select * from cv_requests where  company_id = ? and res_type = ?", [$user['uid'], 'cv'])->rowCount();

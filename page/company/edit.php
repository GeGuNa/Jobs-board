...
$comp_name = my_esc($_POST['comp_name']);
$tel_num = my_esc($_POST['tel_num']);



$www = my_esc($_POST['www']);
$sector = my_esc($_POST['sector']);


$sector = my_esc($_POST['sector']);
$company_identificator = my_esc($_POST['u_idc']);
...
// $_FILES['MyPhoto1']['size'] / 1024  =  1mb


//$_SESSION['qw_1'] = $_FILES;
...
if (isset($_FILES['Cover_pic']['tmp_name']) && text_size($_FILES['Cover_pic']['name'])>=2) {


$picture1z = $_FILES['Cover_pic']['name'];


if (!is_image($picture1z)) { 
...
$img_zp1z = new Imagick($_FILES['Cover_pic']['tmp_name']);
...
if ($_FILES['Cover_pic']['size']  > 1048576*5) {
	...
	$picture_2cov = $_FILES['Cover_pic']['tmp_name'];
   ...
   $ph31_p34zq = my_esc("".$ph31_p4."".$plmblah."_".$time."".rand(0,9999999)."_".rand(0,9999999)."_pic".$ph31_p1);




	
$picturePath1z = $_SERVER['DOCUMENT_ROOT'].'/employers/covers/'.$ph31_p34zq.'.jpg';
convertAndBlur($picture_2cov, $picturePath1z, $img_zp1z->getImageWidth(), $img_zp1z->getImageHeight(), 100);
	...
	$pdo->query("UPDATE hr_user SET cover_pic = ? WHERE uid = ?",[$ph31_p34zq, $user['uid']]);
...
$picturePath = $_SERVER['DOCUMENT_ROOT'].'/employers/images/'.$ph31_p3.'.jpg';
convertAndBlur($picture, $picturePath, $imgewweee->getImageWidth(), $imgewweee->getImageHeight(), 100);




	$pdo->query("UPDATE hr_user SET founded = ?, name = ?, surn = ?, mobnumber = ?, pemail = ?, company_id = ?, compname = ?, website = ?, sector = ?, aboutcompany = ?, country = ?, state = ?, city = ?, postalindex = ?,fulladdress = ?, photo_id = ?, photo_type = ?, facebook = ?, twitter = ?, linkedin = ?, dribble = ?, photo_addr = ?  WHERE uid = ?",[$founded,$name, $surn,$tel_num,$mail,$company_identificator,
	$comp_name,$www,$sector,$about,$country,$state,$city,$postal_code,$f_addr,1251234,'.jpg',$facebook,$twitter,$linkedin,$dribble, $ph31_p3.".jpg", $user['uid']]);
	...
	$pdo->query("UPDATE hr_user SET founded = ?,name = ?, surn = ?, mobnumber = ?, pemail = ?, company_id = ?, compname = ?, website = ?, sector = ?, aboutcompany = ?, country = ?, state = ?, city = ?, postalindex = ?,fulladdress = ?, photo_id = ?, photo_type = ?, facebook = ?, twitter = ?, linkedin = ?, dribble = ? WHERE uid = ?",[$founded,$name, $surn,$tel_num,$mail,$company_identificator,
	$comp_name,$www,$sector,$about,$country,$state,$city,$postal_code,$f_addr,1251234,'.jpg',$facebook,$twitter,$linkedin,$dribble, $user['uid']]);
...
{
error('ფაილის ზომა დიდია: '.size_file($size));
}
...
<div class="container">
		...
		<form action="?" method="post" style="padding:0;" enctype="multipart/form-data">
					...
					<!-- -->	
						...
						<div class="div_50 mrbottom20">
							...
							<input type="text" name="comp_name" value="<?=safechar($user['compname']);?>" minlength="1" maxlength="1000" required="">
							  ...
							  <select name="sector" required>
								...
								$qweqwe = $pdo->query("select * from hr_job_cats");
									while ($qweqwe2 = $qweqwe->fetch()): ?>
										<option value="<?=$qweqwe2['cid'];?>"  <?=($user['sector'] == $qweqwe2['cid'] ? " selected"  : "");?>><?=$qweqwe2['name'];?></option>
								<? endwhile; ?>
							  </select>
		...
		<!-- -->
						...
						<div class="div_50 mrbottom20">
							...
							<input type="number" name="u_idc" value="<?=safechar($user['company_id']);?>">
		...
		<!-- -->			
					...
					<div class="div_100 mrbottom20">
						...
						<input type="number" name="founded" value="<?=safechar($user['founded']);?>">
					...
					<div class="div_100 mrbottom20">
						...
						<input type="file" name="Cover_pic" value="">
